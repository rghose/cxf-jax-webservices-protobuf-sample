...
public class AbeBackend {
	...
	public void getTripsFromAbe() throws Exception {
		...
		Retrofit retrofit = new Retrofit.Builder()
				...
				.build();
		AbeBackendService ab_service = retrofit.create(AbeBackendService.class);
		Call<TripsResponse> ts = ab_service.getTrips(request.build());
		TripsResponse trips = ts.execute().body();
		int count = 0;
		if(trips != null) {
			count = Integer.parseInt(trips.getCount());
		}
		LOGGER.info("Total number of trips: " + count);
	}
}
